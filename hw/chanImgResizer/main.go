...
func (w *Worker) Loop() {
	for {
		// ждем когда данные придут в канал
		t := w.queue.PopWait()
...
func main() {
	...
	for i := 0; i < 50; i++ {
		...
		// горутина заблокируется пока значение не считается.
		// то есть нужно подождать пока до воркера дойдет сигнал по каналу и он сможет считать и только тогда мы сможем запустить следующий воркер?
		// не стоил ли создавать очередь по количеству воркеров ??
